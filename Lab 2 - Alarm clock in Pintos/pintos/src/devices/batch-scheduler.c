...
void getSlot(task_t task) {
    ...
    while (space < 1 || task.direction != currentDirection ||
           (task.priority == NORMAL && (waitingTasks[0][HIGH] || waitingTasks[1][HIGH]))) {
...
void leaveSlot(task_t task) {
    ...
    if (waitingTasks[currentDirection][HIGH]) {
        ...
        if (space == BUS_CAPACITY) { //Only broadcast if bus is free
            cond_broadcast(&waiting[1 - currentDirection][HIGH], &mutex);
        }
